...
get_file_info() {
    ...
    if [ $? -eq 0 ] && [ -n "$duration" ]; then
        # Ensure we use dot as decimal separator for calculations
        duration=$(echo "$duration" | sed 's/,/./g')
        
        # Convert to minutes and seconds for display
        if command -v bc &> /dev/null; then
            local minutes=$(echo "scale=0; $duration / 60" | bc -l 2>/dev/null || echo "0")
            local seconds_raw=$(echo "scale=1; $duration - ($minutes * 60)" | bc -l 2>/dev/null || echo "0")
        else
            # Fallback using awk if bc is not available
            local minutes=$(awk "BEGIN {printf \"%.0f\", $duration / 60}")
            local seconds_raw=$(awk "BEGIN {printf \"%.1f\", $duration - (int($duration / 60) * 60)}")
        fi
        
        # Format the display using locale-appropriate decimal separator
        local seconds_display=$(printf "%.1f" "$seconds_raw" 2>/dev/null || echo "$seconds_raw")
        
        echo "$duration|${minutes}m ${seconds_display}s"
    else
...
do
    ...
    # Prepare text overlays - escape special characters for ffmpeg
    full_header_text=$(echo "$location_text" | sed 's/:/\\:/g' | sed "s/'/\\\\'/g")
    full_date_text=$(echo "$display_name" | sed 's/:/\\:/g' | sed "s/'/\\\\'/g")
    
    # Create the spectrogram movie with proper escaping
    if ffmpeg -i "$full_path" -filter_complex \
        "[0:a]showspectrum=s=996x592:legend=enable:start=$lowest_freq:stop=$highest_freq:fscale=$freq_scale:color=$color_choice:drange=$dynamic_range:scale=$gain_scale:slide=$slide_choice,drawtext=text='$full_header_text':x=25:y=25:fontsize=24:fontcolor=white:box=1:boxcolor=black@0.5:boxborderw=2,drawtext=text='$full_date_text':x=W-tw-25:y=25:fontsize=20:fontcolor=white:box=1:boxcolor=black@0.5:boxborderw=2,format=yuv420p[v]" \
        -map "[v]" -map 0:a \
        -c:v libx264 -preset medium -crf 23 \
        -c:a aac -b:a 128k \
        -movflags +faststart \
        -y "$output_filename" 2>/dev/null; then
